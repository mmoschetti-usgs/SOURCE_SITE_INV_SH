...
C  THIS VERSION IS INTENDED TO RUN WITH CONSTRAINTP.F AND
C  QUARTETPATH.F AND INCLUDES PATH RESPONSE CONSTRAINTS.
C
...
C-----------------------------------------------------------------------
C
      parameter ( nc=6715, nr=17143, nit=512, nbig=55000 )
c
...
C
     ...
     +FORM='UNFORMATTED')
      READ(15) NRA,NCA,NSOR,NSIT,ICREC,IFN,NII,NIIOLD,NIIOLD2
      READ(15) (II(I),I=1,NII)
      CLOSE(15)
      OPEN(UNIT=15,FILE='JJ.DAT',STATUS='OLD',ACCESS='SEQUENTIAL',
     +FORM='UNFORMATTED')
      READ(15) NRA,NCA,NSOR,NSIT,ICREC,IFN,NJJ,NJJOLD,NJJOLD2
